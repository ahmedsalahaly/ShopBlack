...
export default function EditProduct() {
  ...
  const [form, setForm] = useState(
    product ? { ...product, price: String(product.price), stock: String(product.stock) } : null
  );
  ...
  if (!product) {
    return (
      <div className="max-w-[1100px] mx-auto py-8 px-6">
        <p className="text-violet-700 dark:text-violet-300 mb-4">Product not found.</p>
        <button
  ...
  return (
    <div className="max-w-[1100px] mx-auto py-8 px-6">
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <h1 className="text-[1.8rem] font-bold text-[#1e1b4b] dark:text-violet-50 m-0">Edit Product</h1>
        <button
          onClick={() => navigate("/admin")}
          className="bg-transparent border border-violet-400 dark:border-[#3d3d5c] text-violet-700 dark:text-violet-300 px-4 py-2 rounded-md cursor-pointer text-[0.9rem] transition-all hover:border-violet-500 hover:text-violet-500"
        >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input name="price" value={form.price} onChange={handleChange} type="number" step="0.01" className={inputClass} />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input name="stock" value={form.stock} onChange={handleChange} type="number" className={inputClass} />
